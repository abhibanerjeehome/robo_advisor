...
### Get and return Recommended Investment mix
def get_recommendation(risk_level):
    recommendation_response = 'Invalid Option provided for Risk Level, please try again!'

    if risk_level == 'high':
        recommendation_response = '20% bonds (AGG), 80% equities (SPY)'
    elif risk_level == 'medium':
        recommendation_response = '40% bonds (AGG), 60% equities (SPY)'
    elif risk_level == 'low':
        recommendation_response = '60% bonds (AGG), 40% equities (SPY)'
    elif risk_level == '':    
        recommendation_response = '100% bonds (AGG), 0% equities (SPY)'
...
def recommend_portfolio(intent_request):
    ...
    parsed_investment_amount = parse_int(investment_amount)

    response = ''

    is_age_valid = parsed_age >= 0 and parsed_age <= 65

    if not is_age_valid:
       response = build_validation_result(is_age_valid, 'Age', 'Valid age needs to be between 0 and 65 please!')

    is_amount_valid = parsed_investment_amount >= 5000

    if not is_amount_valid:
        response = build_validation_result(is_amount_valid, 'Investment Amount', 'Valid amount needs to be at least 5000 please!')

    is_intent_valid = is_age_valid and is_amount_valid

    if is_intent_valid:
        response = build_validation_result(is_intent_valid, None, get_recommendation(risk_level.lower()))
